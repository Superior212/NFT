...
transaction {
    ...
    prepare(signer: AuthAccount) {
        ...
        let collectionRef = signer.borrow<&PoopArt.Collection>(from: /storage/MyCollection)
        if collectionRef == nil {
            let emptyCollection <- PoopArt.createEmptyCollection()
            signer.save(<- emptyCollection, to: /storage/MyCollection)
            signer.link<&PoopArt.Collection{NonFungibleToken.CollectionPublic}>(
